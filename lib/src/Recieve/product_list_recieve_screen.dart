import 'package:css_stranbys/src/Recieve/product_detail_screen.dart';
...
class ProductListRecieveScreen extends GetView<RecieveController> {
  const ProductListRecieveScreen({super.key});
  ...
  Widget build(BuildContext context) {
    return Obx(() => FIScaffold(
        ...
        child: Column(children: [
          controller.appCtrl.buildSearchField(search_controller: controller.ProductSearchkey),
          _buildScreen(),
        ])));
  }
          ...
          child: Column(
            children: [
              if ((controller.isSearching.isTrue && controller.productSearchData.isEmpty) || (controller.isSearching.isFalse && controller.productData.isEmpty)) ...[
                const FIEmptyState(
                  title: 'No pending recieving list found.',
                )
              ] else ...[
                  ...
                  for (var index = 0; index < controller.productSearchData.length; index++) ...[
                    Bounceable(
                      onTap: () async {
                        ...
                        controller.getImageData(packingLineId: '${controller.productData.value[controller.productDetailIndex.value]?.packing_list_line_id ?? ''}');
                      },
                      child: SizedBox(
                          width: 100.w,
                          child: Stack(
                                ...
                                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                  ...
                                  if (controller.productSearchData.value[index]?.batch_code != null && controller.productSearchData.value[index]?.batch_code != '')
                                    controller.appCtrl.buildKeyValueAlign(
                                        ...
                                        key: 'Batch Code',
                                        value: '${controller.productSearchData.value[index].batch_code ?? ''}'),
                                  ...
                                  //   controller.appCtrl.buildKeyValueAlign(width: 90,color: Colors.green.shade800,key: 'Quantity', value: '${controller.productSearchData.value[index].order_qty ?? ''} / ${controller.productSearchData.value[index].scanned_qty ?? ''}'),
                                  Row(children: [
                                    SizedBox(
                                        width: 90,
